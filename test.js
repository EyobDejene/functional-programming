...
function transformData(data, limit, noR, noBoard, reciprocal, omniLikes) {
    ...
    data.forEach(item => {
        ...
        if (noBoard) {
            item.links = item.links.filter(link => board.indexOf(link) == -1)
        } //Remove board if necessary
        if (noR) {
            item.links = item.links.filter(link => r.indexOf(link) == -1)
        } //Remove rs if necessary
    ...
    if (reciprocal) {
        transformed.links = transformed.links.filter(link => findMatch(link))
    }
